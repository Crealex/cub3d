...
/*   Updated: 2025/07/14 11:01:20 by psoulie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (mapi->cy < 0 || mapi->cx < 0 || mapi->cy >= mapi->len_y
		* data->tilesize || mapi->cx > (int)ft_strlen(mapi->map
			[mapi->cy / data->tilesize]) * data->tilesize)
		put_colour(data, mapi, 0x000000);
	else if (mapi->map[mapi->cy / data->tilesize]
		[mapi->cx / data->tilesize] == '1')
		put_colour(data, mapi, 0x0000FF);
	else if (mapi->map[mapi->cy / data->tilesize]
		[mapi->cx / data->tilesize] == 'O')
		put_colour(data, mapi, 0x12B261);
	else if (mapi->map[mapi->cy / data->tilesize]
		[mapi->cx / data->tilesize] == 'C')
		put_colour(data, mapi, 0x561105);
	else if (is_empty(mapi->map[mapi->cy / data->tilesize]
			[mapi->cx / data->tilesize]))
...
{
	mapi->my = 0;
	mapi->cy = (int)(player->posy - player->half) - (MINIMAP_SIZE / 2
			* data->tilesize);
	mapi->addr = mlx_get_data_addr(mapi->img, &mapi->bpp,
			&mapi->line_size, &mapi->endian);
	while (mapi->cy <= (int)(player->posy + player->half) +
	(MINIMAP_SIZE / 2 * data->tilesize))
	{
		mapi->mx = 0;
		mapi->cx = (int)(player->posx - player->half)
			- (MINIMAP_SIZE / 2 * data->tilesize);
		while (mapi->cx <= (int)(player->posx + player->half) +
		(MINIMAP_SIZE / 2 * data->tilesize))
		{
