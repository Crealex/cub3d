...
/*   Updated: 2025/06/25 22:14:53 by alexandre        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	place_wall(t_data *data, t_hit hit, double offset, double iter)
{
	...
	while (i < wall_size / 2)
	{
		x = (int)((offset + FOV / 2) * data->winsize_x);
			if (x < data->winsize_x )
				*(unsigned int *)(bg->addr + (x * (bg->bpp / 8)) + (data->winsize_y / 2 + i) * bg->line_size) = define_pix_texture(hit, data);//0xA19CB3;//define pix_texture
		i++;
	}
}
